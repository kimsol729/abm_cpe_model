...
#%%
df = pd.read_csv('../data/total_intervention_result.csv')

df_selected = df.iloc[:, 18:22]
def convert_to_list(text):
    return ast.literal_eval(text)

B_clean = np.array([df_selected.iloc[i].values.tolist() for i in range(len(df_selected))])

B_clean_30 = np.array([convert_to_list(B_clean[i][0]) for i in range(len(B_clean))]).reshape(150, 36)
B_clean_60 = np.array([convert_to_list(B_clean[i][1]) for i in range(len(B_clean))]).reshape(150, 36)
B_clean_90 = np.array([convert_to_list(B_clean[i][2]) for i in range(len(B_clean))]).reshape(150, 36)
B_clean_180 = np.array([convert_to_list(B_clean[i][3]) for i in range(len(B_clean))]).reshape(150, 36)
...
# 매트릭스 리스트와 이름
matrices = [B_clean_30, B_clean_60, B_clean_90, B_clean_180]
labels = ['30', '60', '90', '180']

# x축 라벨: '2021-01' ~ '2023-12'
x_labels = pd.date_range(start='2021-01-01', periods=36, freq='MS').strftime('%Y-%m')
...
# x축 설정
plt.xticks(ticks=np.arange(36), labels=x_labels, rotation=45)
plt.title('Cumulative Trimmed Mean (5%~95%) and IQR of B_clean_* matrices (2021–2023)')
plt.xlabel('Month')
