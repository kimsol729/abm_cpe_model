...
# Constants
n<-dim(cumsum_dat)[1];argvals<-seq(1,n,by=1);
obs<-matrix(obs, nrow=1);
cumsum_obs <- cumsum(obs)
nbasis<-15
N.iter<-20000;step.b<-0.005;step.s<-0.1;theta<-as.numeric(colnames(cumsum_dat))
coords<-cbind(theta, rep(0, dim(cumsum_dat)[2]));acc.B<-acc.S<-0
cov.model<-"matern"
...
save(B.samp, E.samp, S.samp, acc.B, acc.S, file="MCMCchain1.RData")
...
if(T){
  ...
  matplot(cumsum_dat, type="l",lty=2,xlab="Month", ylab="Infected", main="Simulation result (A)")
  #matplot(okfd.res$krig.new.data,type="l", ylim=c(0,10),add=T, col="grey", lty=2)
  lines(okfd.res$krig.new.data, col=2, lwd=2)
  lines(t(cumsum_obs),lwd=2,col=3)
}
